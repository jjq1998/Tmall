...
<div class="container" id="basicDiv">
    <div class="row" id="middle">
        ...
        <div class="col-sm-10">
            ...
            <c:if test="${!askIsExist}" var="result2">
                <div id="shopForm">
                    <form action="/shop/submitAsk" enctype="multipart/form-data" method="post">
                        <label for="name">店铺名称</label>
                        <div class="input-group mb-3 custom-control-inline" id="nameInput">
                            <input type="text" class="form-control" placeholder="店铺名称" id="name" name="name">
                        </div>

                        <label for="customFile">店铺图片</label>
                        <div class="custom-file mb-3 custom-control-inline" id="imageInput">
                            <input type="file" class="custom-file-input" accept="image/*" id="customFile" name="shopImage">
                            <label id="labelForImage" class="custom-file-label" for="customFile">选择图片</label>
                        </div>

                        <div id="editor">
                            <label for="kindedito">申请材料</label><textarea name="content" id="kindedito"></textarea>
                        </div>
                        <input type="hidden" name="type" value="开店">
                        <button type="submit" onclick="return submitCheck()" id="ask_btn" class="btn btn-outline-success">
                            提交申请
                        </button>
                    </form>
                    <script>
                        $("#customFile").change(function () {
                            if ($("#customFile").val() !== null) {
                                $("#labelForImage").html($("#customFile").val())
                            }
                        });

                        function submitCheck() {
                            if ($("#customFile").val() == null || $("#customFile").val() == "" || $("#name").val() == null || $("#name").val() == "" || $("#kindedito").val() == "" || $("#kindedito").val() == null) {
                                alert('请确认信息是否正确');
                                return false;
                            }
                        }
            ...
            <c:if test="${!result2}">
                <div class="container" id="askDiv">
                    <p id="askP">您已提交申请，请等待申请结果或&emsp;<a href="/ask/cancelAsk">撤销申请</a></p>
                </div>
            </c:if>
        </div>
    </div>
