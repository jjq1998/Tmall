...
<head>
    ...
    <link rel="stylesheet" href="/css/shop/openShop.css">
...
<div class="container">
    <div class="row" id="middle">
        ...
        <div class="col-sm-10">
            ...
            <c:if test="${shopIsBrand}" var="brandResult">
                <div class="container" id="askDiv">
                    <p id="brandP">您的店铺已通过品牌认证</p>
                </div>
            </c:if>
            <c:if test="${!brandResult}">
                <c:if test="${!askIsExist}" var="result2">
                    <div id="shopForm">
                        <form action="/ask/insertBrandAsk" enctype="multipart/form-data" method="post">
                            <div id="sel1Div">
                                <label for="sel1">品牌主营</label>
                                <select class="form-control custom-control-inline" name="brandType" id="sel1">
                                    <c:forEach items="${typeList}" var="type">
                                        <option value="${type.name}">${type.id}、${type.name}</option>
                                    </c:forEach>
                                </select>
                            </div>

                            <div id="sel2Div">
                                <label for="sel2">品牌选择</label>
                                <select class="form-control custom-control-inline" name="brandName" id="sel2">

                                </select>
                            </div>

                            <div class="form-group" id="brand_name_div" style="display: none">
                                <label for="brand_name">品牌名:</label>
                                <input type="text" class="form-control" name="brandName" id="brand_name"
                                       disabled="disabled">
                            </div>

                            <div class="custom-file mb-3" id="brand_img_div" style="display: none">
                                <input type="file" class="custom-file-input" id="file" name="file" disabled="disabled">
                                <label id="labelForImage" class="custom-file-label" for="file">品牌图标</label>
                            </div>

                            <button type="button" id="brand_btn" class="btn btn-outline-danger" onclick="newBrand()">
                                暂无申请品牌
                            </button>
                            <div id="editor">
                                <label for="kindedito">申请材料</label><textarea name="content" id="kindedito"></textarea>
                            </div>
                            <input type="hidden" name="type" value="认证">
                            <button type="submit" onclick="return submitCheck()" id="ask_btn"
                        ...
                        <script>
                            $("#file").change(function () {
                                if ($("#file").val() !== null) {
                                    $("#labelForImage").html($("#file").val())
                                }
                            });

                            function newBrand() {
                                if ($("#brand_btn").html() == "暂无申请品牌") {
                                    $("#sel2Div").hide();
                                    ...
                                    $("#brand_img_div").hide();
                                    $("#sel2Div").show();
                                    $("#brand_btn").html("暂无申请品牌");
                                }
                            }

                            $("#brand_name").blur(function () {
                                $.ajax({
                                    url: "/ask/selectBrandIsExist",//要请求的服务器url
                                    data: {name: $("#brand_name").val()},//一个对象，表示请求参数，有两个参数 method val  method表示交给Servlet中的什么方法处理，不写默认为doGet或者doPost
                                    async: true,//是否是异步请求
                                    cache: false,//是否缓存结果
                                    type: "POST",//请求方式
                                    dataType: "json",//服务器返回什么类型数据 text xml javascript json(javascript对象)
                            ...
                            $("#sel1").change(function () {
                                $.ajax({
                                    url: "/ask/selectBrandByTypeName",//要请求的服务器url
                                    data: {typeName: $('#sel1 option:selected').val()},//一个对象，表示请求参数，有两个参数 method val  method表示交给Servlet中的什么方法处理，不写默认为doGet或者doPost
                                    async: true,//是否是异步请求
                                    cache: false,//是否缓存结果
                                    type: "POST",//请求方式
                                    dataType: "json",//服务器返回什么类型数据 text xml javascript json(javascript对象)
                ...
                <c:if test="${!result2}">
                    <div class="container" id="askDiv">
                        <p id="askP">您已提交申请，请等待申请结果或&emsp;<a href="/ask/cancelBrandAsk">撤销申请</a></p>
                    </div>
                </c:if>
            </c:if>
        </div>
