        ...
        <div class="col-sm-10">
            ...
            <form action="#" enctype="multipart/form-data" method="post">
                <div class="row">
                    <div class="col-sm-6">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name="name" id="goods_name" value="${goods.name}">
                        </div>
                        ...
                        <div class="custom-file mb-3 custom-control-inline" id="imageInput">
                            <input type="file" class="custom-file-input" accept="image/*" id="goodsImage"
                                   name="goodsImage">
                            <label id="labelForImage" class="custom-file-label" for="goodsImage">${goods.image}</label>
                        </div>
                        <div class="form-group">
                            <label for="goods_price">商品价格：</label>
                            <input type="text" class="form-control" name="price" id="goods_price" value="${goods.price}">
                        </div>
                ...
                <div id="goodsType_div">
                    <c:forEach items="${goods.goodsTypes}" var="type">
                        <button type='button' id='btn${type.id}' onclick='deleteType(${type.id})' class="btn btn-outline-info">${type.name}</button>
                        <input type='hidden' id='${type.id}' name='typeId' value='${type.id}'>
                    </c:forEach>
                </div>
                <hr style="margin-bottom: 10px">
                ...
                <div id="editor">
                    <label for="kindedito">商品详情</label><textarea name="description" id="kindedito">${goods.description}</textarea>
                </div>
                <button id="submit_btn" type="button" class="btn btn-success" style="float: right; margin-top: 10px">上传商品</button>
            </form>
            <script>
                ...
                $(function () {
                    $.each(details, function (i) {
                        $("#param_input_div").append(
                            ...
                            "<label for='" + i + "'>" + i + "</label><input type=\"text\" id=" + i + " name=" + i + " value='" + details[i] + "' class=\"form-control param_class\">\n" +
                            "</div>"
                        );
                    });
                });
                ...
                $("#submit_btn").click(function () {
                    ...
                    $.ajax({
                        url: "/goods/updateGoods",//要请求的服务器url
                        data: formData,
                        async: false,//是否是异步请求
                        cache: false,//是否缓存结果
                        type: "POST",//请求方式
